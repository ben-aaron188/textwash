...
if __name__ == "__main__":
    ...
    anonymizer = Anonymizer(config, model, data_processor, device, bert_model)

    outputs = {}

    for idx, (k, text) in enumerate(data.items()):
        anonymized, orig_cut = anonymizer.anonymize(text)
        outputs[k] = {"orig": orig_cut, "anon": anonymized}
